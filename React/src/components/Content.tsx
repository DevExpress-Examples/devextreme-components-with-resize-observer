...
function dataGridInitialized({ component, element }: DataGridTypes.InitializedEvent): void {
...
function chartInitialized({ component, element }: ChartTypes.InitializedEvent): void {
  if (component && element?.parentNode instanceof Element) {
...
const Content: React.FC<ContentProps> = React.memo(({ selectedIndex, selectedIndexChange }) => {
  const handleTabChange = useCallback(
    (event: React.SyntheticEvent, newValue: string): void => {
      selectedIndexChange(event, newValue);
    },
    [selectedIndexChange],
  );
  ...
  return (
    <React.Fragment>
      ...
      <TabContent value="1" selectedIndex={selectedIndex}>
        <Chart id="chart" onInitialized={chartInitialized} dataSource={populationData}>
          <Series valueField="val" argumentField="arg" name="Population" type="bar" color="#ffaa66" />
        </Chart>
      </TabContent>
      <TabContent value="2" selectedIndex={selectedIndex}>
        <Accordion defaultActiveKey="0">
          <Accordion.Item eventKey="0">
            ...
            <Accordion.Body>
              <HtmlEditor height="400px" onInitialized={htmlEditorInitialized} defaultValue="">
      ...
      <TabContent value="3" selectedIndex={selectedIndex}>
        ...
        >
          ...
          <Summary>
            <GroupItem column="OrderNumber" summaryType="count" displayFormat="{0} orders" />
          </Summary>
        </DataGrid>
      </TabContent>
    </React.Fragment>
